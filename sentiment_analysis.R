...
remove_reg <- "#[a-z,A-Z]*|@[a-z,A-Z]"
assess_sentiment <- tweets_output4_oscars %>% 
  filter(!str_detect(tweet_text, "^RT")) %>%
  mutate(tweet_text = str_remove_all(tweet_text, remove_reg)) %>%
  unnest_tokens(word, tweet_text, token = "tweets") %>%
  filter(!word %in% stop_words$word,
         !word %in% str_remove_all(stop_words$word, "'"),
         str_detect(word, "[a-z]"))

afinn <- assess_sentiment %>% 
